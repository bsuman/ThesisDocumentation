...
\begin{columns}
    ...
    \begin{itemize}
			...
			\item Larger print objects are composed of huge number of voxels( can easily cross \begin{math}10^{12}\end{math} per object)
		\end{itemize} 
    \end{column}
  \end{columns}
\end{frame}
...
\begin{columns}
   ...
   \end{column}
    ...
    \centering
    \begin{itemize}
		\item Distribution of one large print object computation amongst the workstations 
		\item Distribution of multiple print objects computation amongst the workstations
		\end{itemize} 
    \end{column}
  \end{columns}
  \end{frame}
...
\begin{itemize}
	...
	\item Total Volume(for cluster size n-1)=\begin{math}\sum\limits_{i=1}^{n-1}{V^i}\end{math}
	\item Threshold=\begin{math}(\sum\limits_{i=1}^{n-1}{V^i})/(n-1)\end{math}
	\item Each slave node is allocated objects until the sum of the allocated objects does not exceed the calculated threshold
\end{itemize}
\end{frame}		
...
\begin{frame}{Slave Reporter}
	\begin{itemize}
		\item Each Slave node after performing the computation has chunk-wise partial output inform of slices 
...
\item Various parameters other than the volume of the objects can be used to determine the division of workload  
...
\begin{frame}{Serialize \& De-serialize Print Jobs }
	\begin{itemize}
		...
		\item Serialized PrintJob can be instead sent to the slave nodes which would then de-serialize and proceed with the computation 
	\end{itemize} 
\end{frame}
...
\begin{frame}{Performance Gain and management over-head}
	\begin{itemize}
...
\begin{frame}
  \frametitle<presentation>{Summary}
	\begin{itemize}
		...
		\item Supporting the streaming architecture of cuttlefish, develop an elegant solution to achieve the goal
